...
void lcdws(char *s){
	while (*s) {
		...
		#else
			switch(lcdptr) {
				case 0x80+16: lcdptr = 0xC0; break;
				case 0xC0+16: lcdptr = 0x80; break;
			}
		#endif
		lcdwc(lcdptr++);
		lcdwd(*s++);
...
void lcddinit(void) {
	...
	for ( j = 0; j < LCD_NSCRL; j++ ) {
		adr = 0xc0;						// 2nd row, first coloum
		for ( i = 0; i < 16; i++ ) {
			lcdwc(adr);			
			lcdwd(LCD_DOT);				
			if ( i < 8 ) dlyms(100+(50*i)); else dlyms(50);
			lcdwc(adr);			
			lcdwd(LCD_SPC);			
			adr++;					// increment display address
		}
	} 
}
...
void lcdclrr(int8u rowno){
 ...
 void lcdwsint(int8u loc,signed int x) {
	 int8u str[10];
	 lcdptr = loc;
	 lcdws("   ");
	 lcdptr = loc;
 ...
 void lcdwint(int8u loc,uint16_t x) {
	 int8u str[24];
	 lcdptr = loc;
	 ltoa(x, str);
	 lcdws(str);
